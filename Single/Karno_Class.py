...
def Init_Images():
    for i in range(3):
        for j in range(8):
            img[i][j]=tk.PhotoImage(file="pic/{0}/{1}.png".format(i,j))
...
def Init_Board(env):
    ...
    if debug:
        for i in range(1,119):
            blk=block[i]
            if blk.boundary or True:
                env.canvas.create_text(blk.sx,blk.sy,text="{0},{1},{2}".format(blk.X,blk.Y,blk.Z))
                # env.canvas.create_text(blk.sx,blk.sy,text="{0}".format(blk.type2))
...
class Chess:
    ...
    def Show(self,env):
        ...
        if(self.type1==1):
            bias=-12
        self.pic=env.canvas.create_image(sx,sy+bias,image=img[self.group][self.ocp])
...
class Karno:
    ...
    def Show_Reachable_Block(self,env):
        for blk in block:
            if blk==0:
                continue
            if self.choose==0:
    ...
    def Show_King_in_Crisis(self,env):
        king=chess[king_in_crisis]
        blk=block[king.B_id()]
        blk.color="red"
        
    def Show_Swapable(self,env):
        king=chess[king_in_crisis]
        t2=[M1,F1,H1]
        gp=king.group
        for chs in chess:
    ...
    def Show_Chess(self,env):
        ...
        self.Set_Protection()
        self.Clear(env)
        self.Show_Protection(env)
        if king_in_crisis==0:
            self.special_rule_king=-1
            self.Show_Killable(env)
            self.Show_Choosen(env)
            self.Show_Reachable_Block(env)
        else:
            self.special_rule_king=chess[king_in_crisis].group
            self.Show_King_in_Crisis(env)
            self.Show_Swapable(env)
